...
- **Supporting data point (A4 Phase 3, 2026-07-15):** of the petty-cash `cash_transactions` with no `project_id`, 23 are "out" (draws/expenses), not just the 3 expected "in" top-ups. If any of those 23 are real client-billable expenses that were never tagged with a project, they're invisible to any per-project cost view today — concrete evidence for the gap above, not just a theoretical one. **User's call, flagged for A7, not resolved here:** worth a manual skim of which of those 23 are actually billable before deciding how big this gap really is.
