...
> *(Revised: 2026-07-16 (g) — ✅ **`20260717` applied (user-confirmed) + full Phase 2 re-test PASSED.** All manager group-member scoping works: a manager now sees/searches same-group members in the time pages and can approve their leave/flex/expense; same-group admins/managers show names only. **All A4 phases now green: Phase 1 ✅, Phase 2 ✅ (this round), Phase 3 ✅ (2026-07-15), and the only remaining phase is Phase 4 (UI/UX + CSP console check).** One more re-test find, fixed same session: **flex-swap substitute date couldn't be set in the past** (the `min="today"` on `#hl-flex-sub` blocked recording a swap for a holiday that already occurred — the holiday dropdown + submit already allowed past holidays; only that date field blocked it). Removed the min (weekend guard + approver remain), **v=128** merged to `main`. **Migrations: all R63 migrations now applied** (`20260716`, `b`, `c`, `d`, `20260717`) — nothing pending Studio. **▶ Next: Phase 4** — the last A4 phase, then A7 team review → A8 roster swap. Non-blocking audit follow-ups still logged (manager UPDATE-policy breadth, UI-only billable/maternity gating, small cleanups).)*
> *(Revised: 2026-07-16 (f) — 🔧 **R63 round-2 fixes from the user's continued testing (they haven't run the formal re-test yet).** Three reports: (1) **calendar day-view date selector showed the whole week** — `datesSet` reused the week-view label branch for `timeGridDay`; now shows the single day. (2) **My Balance still showed a Maternity Leave card for male/unset employees** — only the request-type dropdown had been gender-filtered in v=126; the balance *cards* had not. Now maternity is excluded from the balance entitlement set unless `gender='female'`. Both shipped in **v=127** (`calendar.js`, `holidays-my-leave.js`; merged to `main`). (3) **Manager couldn't see or search a same-group member's time entries** — root cause: `20260716` scoped managers to *direct reports* (`is_my_report`), but the Team page shows the whole *group* (`shares_group`), so a same-group non-report was correctly hidden from time data. **User defined the authoritative access model:** owner/admin = all; **manager = own + same-group `role='member'`** (see + approve), can see same-group admin/manager *names* (Team page) but not their data; **member = own only, no approve.** New migration **`20260717_manager_group_member_scope.sql`** (⚠️ PENDING Studio — supersedes `20260716`'s manager policies) implements it: `shares_group_member()`/`shares_group_member_emp()` helpers, rewrites the 6 `*_scope` SELECT/UPDATE policies to the same-group-member predicate, and adds an `employees` SELECT policy so the time-page employee picker lists same-group members (was empty for managers → "can't search the name"). No JS/cache change for #3 (picker fills once RLS allows). **▶ Next: apply `20260717` in Studio (dump `%_scope` + `employees` policies first), then the re-test (manager sees/approves same-group members' time/leave/expense; admins/managers in-group show names only).**)*
> *(Revised: 2026-07-16 (e) — ✅ **`20260716d_manager_create_projects.sql` applied in Studio (user-confirmed).** All 4 R63 migrations are now live (`20260716`, `20260716b`, `20260716c`, `20260716d`). Managers can now create projects. Nothing R63-related is pending Studio anymore — remaining work is purely the live re-test pass (6 items in `PHASE_2_WALKTHROUGH_GUIDE.md`) → then Phase 4 (UI/UX + CSP), the last A4 phase.**)*
> *(Revised: 2026-07-16 (d) — 🔧 **R63 follow-up: re-checked the filled walkthrough sheet for notes not yet folded in — found 3, all now dispositioned.** (1) **Manager "can't create a new project"** — the CREATE NEW PROJECT button already shows to managers (`canCreate = isAdmin() || isManager()`), but the DB INSERT was admin-only, so it failed. **User's decision: let managers create** (widens the earlier "self-assign only" scope for creation only; edit/archive of others' projects stays admin-only). New migration **`20260716d_manager_create_projects.sql`** (⚠️ PENDING Studio — dump `projects` `pg_policies` first) adds a PERMISSIVE `projects` INSERT policy for `get_my_role()='manager'`; no JS/cache change (button already visible). (2) **Member "can access the employee page"** — confirmed **by-design**: that's the Team page (R61 same-group scoping); `#employees` (admin Directory) is gated `() => isAdmin()` in `routeAllowed` and its nav item hidden for non-admins, no leak. (3) **Admin "Clients/Projects code inputs [Fail]"** — re-traced: `code` IS mapped through `createClient`/`updateClient`/`createProject`/`updateProject` and the unique indexes exist, so save + duplicate-reject both work; the [Fail] was the raw Postgres duplicate-key error text, already fixed in v=126 by the friendly-toast mappers. No further code change. **▶ Re-test list in `PHASE_2_WALKTHROUGH_GUIDE.md` now has a 6th item (manager creates a project, after `20260716d`).**)*
> *(Revised: 2026-07-16 (c) — ✅ **R63 fully deployed: all 3 migrations applied in Studio (user-confirmed "all success") + v=126 merged to `main` and live.** Merge `50ff554` also carried one post-audit fix: a code audit (8-angle review, partially degraded by an org spend-limit killing 5 finder agents — those angles were completed inline) found that Calendar/Timesheet still defaulted to an unfiltered entries query, which after `20260716`'s RLS would have blended every visible user's entries into admin/manager default views — fixed like the tracker (`_viewUserId || _profile.id`; `calendar.js`/`timesheet.js`). **Audit's remaining non-blocking findings, logged for a future round:** (1) the new manager UPDATE policies grant full-row update on reports' leave/flex/expense rows — broader than approve/reject needs; decide accept vs. tighten (column guard/trigger). (2) billable + maternity gating are UI-only; a member could still PATCH `is_billable` or POST a maternity request via raw REST — decide whether either needs a DB-side backstop. (3) cleanups: duplicate `_friendlyClientError`/`_friendlyProjectError` should be one shared helper keyed on `err.code === '23505'` (documents.js already does it that way); `_toggleMyManager` re-hydrates ALL project stats to flip one button; `getMyManagedProjectIds` wastes an `auth.getUser()` round-trip + runs sequentially after the `Promise.all`; employee-edit audit log uses `{changed:[names]}` while client edits use `{fields:{old,new}}` — unify; calendar.js duplicates weekNav's date-picker plumbing verbatim. **▶ Next: the 5-item re-test list at the top of `PHASE_2_WALKTHROUGH_GUIDE.md`** (manager team rows + approvals end-to-end, admin cross-user calendar/timesheet via the picker, maternity female-only, mileage total-distance, duplicate customer PN on a *manual*-mode project) **→ then Phase 4 (UI/UX + CSP console check), the last A4 phase.**)*
> *(Revised: 2026-07-16 (b) — 🔧 **R63: live Phase 2 walkthrough results folded in — user ran the full 2A–2I click-through on prod and returned a filled sheet; all findings root-caused and fixed. Cache JS `?v=125→126`.** Branch `claude/phase-2-static-audit-66040`. **11 client-side fixes shipped:** (2A) **Calendar prev/next** were wired through `weekNav`'s fixed ±7-day Monday snap → "next month" looked dead and day view jumped Monday-to-Monday; rewired to FullCalendar's view-aware `prev()`/`next()` (`calendar.js`). (2A) **Billable checkbox/`$` toggle** now admin/manager-only (`entryModal.js`, `tracker.js`) — members' entries default billable. (2A) **Time Tracker** now defaults to "Myself" (`tracker.js` `_loadEntries` uses `_profile.id`, not unfiltered) so admins don't see everyone by default once the RLS below lands. (2B) **Leave year selector** 3→5 years (`holidays-my-leave.js`). (2B) **Balance cards** no longer swap position — `getAllLeaveBalances` now `.order('leave_type_code')` + a stable sort in `_balCards` (`leaves.js`, `holidays-state.js`). (2B) **Maternity leave** hidden unless `employees.gender='female'` (`holidays-my-leave.js`; gender added to the 3 myEmployee selects + employee modal + employees API). (2C) **Mileage round-trip ×2 removed** — entered distance is the total (`api/expenses.js previewMileage`; server trigger fixed by migration below). (2F) **Employee field edits** now write an Admin Logs entry (`employees.js` diff → `logAction('update_employee', …)`). (2H) **PN category** now a required deliberate choice — disabled "— Select category —" placeholder + save-time guard (`partNumbers.js`). (2H) **CCC/PPP duplicate-code** errors now show a human toast instead of raw Postgres text (`clients.js`, `projects.js`). (2I) **Manager self-assign** — managers get a per-row Join/Leave-as-manager button on Projects (they couldn't reach the admin-only assign modal); calls `assignManager`/`unassignManager` with their own uid, which the existing `pa_insert` policy already permits (`projects.js` + new `getMyManagedProjectIds()`). **3 migrations scaffolded — ⚠️ PENDING Studio apply (this container can't reach prod):** `20260716_manager_scope_time_and_requests.sql` (**the big one** — root cause of the manager seeing ZERO team time/leave/flex/expense rows: no manager→direct-report RLS on those 6 tables; adds `is_my_report_emp()`/`my_employee_id()` + additive SELECT+UPDATE policies; **dump `pg_policies` first** to reconcile names/keys), `20260716b_employee_gender.sql` (**apply BEFORE deploying v=126** — the `.select(gender)` calls error if the column is absent), `20260716c_mileage_total_distance.sql` (dump-first template — the trigger body isn't in the repo; strip the ×2 and re-apply). **Won't-fix / by-design (docs corrected):** petty-cash stays admin-only (user decision — checklist was wrong); client bounced from `#part-numbers` to the portal is correct (better than empty data); managers editing/archiving others' projects stays admin-only (only self-assign added). **Duplicate customer PN "generated the same number":** not a bug — the internal `CCC-PPP-CAT-SEQ` is always freshly minted; customer-PN dedupe only fires in `manual` customer-PN mode. The test project was almost certainly in `none` mode → re-test with a `manual`-mode project. **`check:parse` 56/56.** **▶ Next: user applies the 3 migrations in Studio (order: 20260716b before frontend push), deploys v=126, then re-tests the migration-dependent rows — manager approvals/team calendar (C1), maternity visibility (C2), mileage total (C3) — and the manual-mode duplicate PN. Then Phase 4 (UI/UX + CSP).**)*
> *(Revised: 2026-07-16 — 🔍 **A4 Phase 2 (2A–2I) run as a STATIC CODE AUDIT, not a live browser walkthrough** — this container has a hard 403 CONNECT-tunnel denial to `surasaknie.github.io` (confirmed via the proxy status endpoint, same restriction as every prior round), so 9 parallel code-reading passes traced each checklist section against the actual page/API/RLS source instead. **Clean, no findings:** 2B (2-tier leave, flex swaps, holidays CRUD, balance tab), 2C (expenses/petty cash), 2E (clients/documents/reports — R61 rate-gating for managers reconfirmed), 2I (R61 team scoping + the `project_assignments` trigger fix, confirmed scoped to that table only). **One false-positive caught and walked back:** 2H's agent flagged `pn_create_item` as "burning" the sequence counter on a duplicate customer PN — checked the actual SQL (`20260711_part_numbers_v2.sql:254-263`): the exception handler re-raises via `RAISE EXCEPTION`, which is uncaught and aborts the whole RPC transaction (one Supabase RPC call = one transaction), rolling back the earlier counter increment too. Not a bug — R55's Lists-button fix independently reconfirmed still fixed. **4 real gaps found, none blocking, all new to the backlog:** (1) **2A** — no "submit for approval" step exists on timesheet entries anywhere in the code (they save as final immediately), and no standalone WFH toggle (WFH only reachable via the flex-swap form) — confirm both are intentional design, not missing features, before treating as bugs. (2) **2D** — name-change/job-title **reject and cancel** bypass the atomic F-05 RPCs and write directly to the request tables (only *approve* is RPC-based); also no avatar-edit UI exists at all (name is request-flow-only by design, avatar has no UI path whatsoever). (3) **2F** — plain employee field edits (name/title/salary) get no Admin Logs entry, only account-state actions (provision/reset/deactivate/reactivate) do — confirm whether the separate trigger-based `employee_audit_log` table is meant to be the record of these instead. (4) **2G** — freeform fields (expense notes, travel destination) pass through to the client portal/export unsanitized; if staff type an employee's name into a note it leaks to the client with no redaction — soft/process risk, not an RLS bug. **Audit blind spot, not a confirmed bug:** several agents couldn't find RLS policy definitions for `time_entries`, `leave_requests`/`leave_balances`, `clients`, or `name_change_requests` anywhere in this repo's tracked SQL — almost certainly because this repo only started tracking migration files as of `20260629`; earlier schema/RLS was applied directly via Studio and was never committed here, which is a repo-history gap, not evidence the policies are actually missing in prod. **Flagging 2A's and 2B's "manager sees all leave, not just direct reports" claim as UNVERIFIED**, pending a live Studio policy check (`pg_policies` for `leave_requests`/`leave_balances`), same pattern already used to verify `profiles_select` for R61 — do not treat as a confirmed leak until checked live. **▶ Next:** live Studio RLS check on `leave_requests`/`leave_balances`/`time_entries`/`clients` (quick, Studio-only) → decide on the 4 gaps above (bugs vs. by-design) → Phase 4 (UI/UX + CSP) → whenever network access allows, a real browser click-through still owed before go-live sign-off, since this round was code-only.)*
> *(Revised: 2026-07-15 (f) — ✅ **A4 Phase 3 done live (user running 3 → 2 → 4).** Core integrity: 6/7 clean 0-rows (negative leave balances, orphaned time entries, invalid leave status, duplicate leave balances, client profiles missing client_id, orphaned project_assignments). **#4 (cash transactions with no project) deliberately not scored PASS/FAIL** — checked `js/api/expenses.js`: `project_id` is genuinely optional on every insert/update, so this isn't a bug. Breakdown: 3 "in" rows (round-number top-ups, 6000/10000/2000 THB) with no project is expected and fine. **23 "out" rows with no project is flagged for 🧑's judgment** — if any are real client-billable expenses that just weren't tagged, they're invisible to any per-project cost view, which is the same gap flagged in the B1/invoicing conversation; if they're genuine overhead, no action needed. **User's call: log it, don't drop it** — folded into `FULL_EXECUTION_PLAN.md`'s B1 open-question callout as a concrete supporting data point (not resolved there either — still needs a manual skim of which of the 23 are actually billable before the CEO/team can judge the gap's real size at A7). Also flagged one outlier: a ~456,123 THB draw, ~45× the next-largest amount — worth a manual look regardless of the project question. **🐛 Found + fixed a real bug — in 🤖's own R62 query, not the app:** the `#7` orphaned-`project_assignments` check referenced `pa.id`, but that table has no surrogate id column (only `project_id`+`manager_id`, confirmed against `js/api/projects.js` which never selects `.id` on it) — threw `42703` live. Fixed in both audit docs (dropped `pa.id` from the SELECT; join/filter logic never needed it), re-ran: **0 rows**, confirmed clean. **Part Numbers P1–P4: all 4 clean 0-rows.** **Phase 3 is now fully done. ▶ Next (user's order): Phase 2 (browser walkthrough, incl. 2I) → Phase 4 (UI/UX + CSP).**)*
> *(Revised: 2026-07-15 (e) — ✅ **A4 Phase 1H done live — clean regression, no findings.** All 3 F-05 RPCs still present (`approve_deletion_request`, `review_name_change_request`, `approve_job_title_change_request`), no drift since the 2026-06-30 verification. **This closes all of Phase 1 (1A through 1H). ▶ Next: Phase 2 (browser walkthrough, incl. 2I) → Phase 3 (Studio SQL integrity) → Phase 4 (UI/UX + CSP).**)*
> *(Revised: 2026-07-15 (d) — ✅ **A4 Phase 1F done live (Studio SQL, all 3 sub-blocks green) — found + fixed 2 doc-naming errors along the way.** Core-policy check initially came back 6/9 (3 "missing"): `jtcr_own_select`, `jtcr_own_insert`, `audit_log_insert_own` weren't found. Ran the packet's own built-in fallback (broader per-table policy discovery) rather than treating it as a real gap first — same lesson as the RPC that turned out to be `review_name_change_request`, not `approve_name_change_request`. **All 3 exist, just named differently:** real names are `jtcr_select_own`, `jtcr_insert_own`, `audit_log_insert` (word-order/suffix swapped from what the doc guessed). Confirmed `audit_log_insert`'s `with_check = (actor_id = auth.uid())` — matches 1G's behavioral PASS exactly. **Part Numbers:** 16/16 policies, one RESTRICTIVE `client_block_*` per table, 0 INSERT policies on `pn_items` (positive control) — and `pn_items`' own policy list (delete/select/update, no insert) independently confirms the same thing. **R61:** `profiles_select`'s `qual` is a byte-for-byte match to the migration's intended predicate (not just "references" the helpers — structurally identical); all 3 helper fns present, `search_path` pinned on each. **Trigger-fix regression:** query returned 2 rows, both `task_assignments` — this is expected Postgres behavior (`information_schema.triggers` gives one row per event for a multi-event `BEFORE INSERT OR UPDATE` trigger), not a discrepancy; zero `project_assignments` rows confirms the 20260713b fix is live. **Docs corrected** (packet + plan + pass-criteria table): the 3 real policy names, and the trigger-check wording changed from "expect exactly 1 row" to "expect only `task_assignments` rows (2 is normal), zero `project_assignments`" so it can't misread as a fail next time. **1F is now fully done. ▶ Next: 1H (Studio SQL, one query, quick) → Phase 2 (browser, incl. 2I) → Phase 3 → Phase 4.**)*
> *(Revised: 2026-07-15 (c) — ✅ **A4-F2 CLOSED — Phase 1E + 1G are now fully done.** Traced the CORS finding to its source: no shared cors module, `ALLOWED_ORIGINS` is a literal array copy-pasted into each of the 7 Edge Functions (confirmed identical byte-for-byte across all 7 via `supabase functions download`). Fixed all 7 in one pass — removed `https://he-cells.github.io`, kept `localhost:3030` for local dev, promoted `https://surasaknie.github.io` to the fallback (`[0]`) slot — then `supabase functions deploy` on all 7. Re-ran `edge_probe.ps1`: **18 PASS / 0 FAIL / 0 WARN**, every old-origin CORS check now explicitly reads "not echoed." Combined with the prior credentialed run (22 PASS covering non-admin-403 + admin-malformed-input + the 1G spoof — unaffected by this deploy since only the origin list changed, not auth logic), **every 1E/1G check has now passed at least once.** Side note: this required getting the Supabase CLI authenticated (`supabase login`) and linked (`supabase link --project-ref sjkggguedgtynktymzes`) on a fresh local clone — worth remembering next time function source is needed, since it's gitignored and was never in any checkout. **▶ Next: 1F/1H/3 (Studio SQL, fully independent of Edge Functions/rate-limits) → Phase 2 (browser, incl. 2I) → Phase 4.**)*
> *(Revised: 2026-07-15 (b) — ✅ **A4 1E + 1G checks PASS live (credentialed `edge_probe.ps1` re-run — 22 PASS / 0 WARN on all non-CORS checks).** The login rate-limit had reset (input-validation back to clean 401s: wrong-pw, non-existent-identical-body, oversized, no-auth). **All token-gated checks green:** the 6 admin-only Edge Functions each return 403 to a member token, and **1G** — a spoofed `actor_id` insert into `audit_log` — returns 403 (`WITH CHECK actor_id = auth.uid()` holds). Admin-malformed-input: 4/4 return 400, no 500s. **So 1E's checks + all of 1G are complete; 1E's only remaining item is A4-F2** (remove the dead `he-cells.github.io` origin from all 7 functions' `ALLOWED_ORIGINS` + redeploy, then re-run to flip the 7 old-origin CORS FAILs to ok). Minor process note: the sci-fi test creds used for the run get wiped at the A8 roster swap, so pasting them was harmless — but keep real-roster passwords out of chat/logs as a habit. **▶ Next: A4-F2 CORS fix; plus 1F/1H/3 (Studio SQL) + Phase 2/4 (browser), all independent of the login limiter.**)*
> *(Revised: 2026-07-15 — 🔍 **R62 cont.: A4 Phase 1E/1G run live against prod (user, `edge_probe.ps1`). 1 real finding — A4-F2 (CORS).** Input validation: ✅ wrong-password → 401, non-existent ID → identical 401 body (no user-enumeration leak — the security-relevant check), no-auth → 401. Oversized payload → 429 (WARN, acceptable — not a 500 crash; it's the login rate-limiter). CORS new origin `surasaknie.github.io` → ✅ 7/7. **🔴 A4-F2 (CORS, Low severity, fix before go-live): the dead `he-cells.github.io` origin is STILL whitelisted in `ALLOWED_ORIGINS` on all 7 Edge Functions** (`login`, `provision-users`, `admin-reset-password`, `admin-set-account-active`, `admin-clear-mfa`, `account-activation-status`, `provision-client`) — R53 added the new origin but never removed the old one. Low severity (that Pages site is dead; CORS gates response-readability, not auth) but the audit bar is "old origin must not echo," and the fix is trivial: remove `https://he-cells.github.io` from each function's `ALLOWED_ORIGINS` (or the shared `_shared/cors.ts`), `supabase functions deploy` all 7, re-run `edge_probe.ps1` → the 7 old-origin FAILs flip to ok. **User-side action** — function sources are gitignored + deploy via the Supabase CLI, not from this container. **Test artifact, not a bug:** login returned 429 on the second probe run because the probe hit `/login` repeatedly — brute-force rate-limiting working as intended; wait for the window to reset before re-running so login checks read cleanly. **Still pending (skipped — literal placeholder creds were passed):** the 7 non-admin-403 checks, 4 admin-malformed-input checks, and the **1G audit-log spoof test** — re-run `edge_probe.ps1` once with a real non-admin token (e.g. manager `02-3-003-56`, already known-good this session) + a real admin login. **▶ Then continue A4:** 1F/1H/3 (Studio SQL) → Phase 2 (browser, incl. 2I) → Phase 4 (UI/UX + CSP). Also, unrelated: the runbook artifact's run-location banners are now colour-coded (amber=terminal, Supabase-green=Studio, pink=browser) so the curl-vs-SQL mixup that bit the first 1E attempt can't recur.)*
> *(Revised: 2026-07-13 — 📋 **Round 62: A4 audit-packet coverage caught up to R61 (docs + scripts only, no cache change).** 1A–1D are done and the R61 team-visibility detour is done (both per the entries below); this session took the literal next-step ask — "A4's remaining phases 1E/1F/1G/1H/2/3/4" — and found the audit materials for those phases predate R61's schema changes (new `profiles_select` policy/helper fns, the `project_assignments` trigger fix), so brought them current before anyone runs them live. **`PRE_LAUNCH_AUDIT_PLAN.md` + `PRE_LAUNCH_AUDIT_EXECUTION_PACKET.md` Phase 1F** gained a SQL block: `profiles_select` must show the new role-scoped `qual` (not the old blanket clause); `shares_group`/`is_my_report`/`is_client_on_my_projects` must exist with `search_path` pinned; `information_schema.triggers` must show `check_assignment_role` attached to `task_assignments` only (a project_assignments hit means the 20260713b fix never applied and the Projects Managers section will still throw). **New `2I · Team & Projects` Phase-2 subsection** — this page had **zero** Phase-2 walkthrough coverage before, independent of R61: member Team page = same-group staff, 0 clients, no rate column; manager Team page = same-group + reports + read-only project-clients only; admin = everyone; Projects assign modal's Managers toggle writes without the old `assignee_type` crash; assigning a manager surfaces that project's client on their Team page (positive-control follow-through). **Phase 3** gained an orphaned-`project_assignments` query (same shape as the existing orphaned-time_entries check) — expected 0 rows, though real rows will start appearing once 2I's walkthrough (and general use) starts assigning managers. Pass-criteria table and `FULL_EXECUTION_PLAN.md`'s A4 status/what-text updated to match. **Verified no drift in 1E**: grepped the actual `${EDGE}/...` call sites in `employees.js`/`clients.js`/`login-init.js` — still exactly the 7 functions `edge_probe.ps1` already tests (`login`, `provision-users`, `admin-reset-password`, `admin-clear-mfa`, `admin-set-account-active`, `account-activation-status`, `provision-client`). Re-confirmed via the proxy status endpoint that this container still gets a hard 403 CONNECT-tunnel denial to both `*.supabase.co` and `*.github.io` — none of 1E/1F/1G/1H/2/3/4 could be executed from here; this round only makes sure the next live run doesn't hit the kind of stale-target surprise 1A hit (`anon_probe.scratch.ps1` never existing). **Correction, same day:** the line above (and CLAUDE.md's/`FULL_EXECUTION_PLAN.md`'s versions of it) said R61 was "unmerged, awaiting go-ahead" — that was stale. Checked `origin/main` directly instead of trusting the committed doc text: **`main` already has R61** (`js/app-init.js` on `main` reads `v=125`; `origin/main`'s tip is the exact commit this branch started from). It's merged and live, not an open item. **▶ Next:** 🧑/🌐 runs 1E (`edge_probe.ps1`) → 1F/1G/1H/3 (Studio SQL, now current) → Phase 2 walkthrough (now incl. 2I) → Phase 4 (UI/UX + CSP console check) → report pass/fail back. **Same-session follow-up (user-directed):** Track A's **A6** (closeout content) **moved to Track B as `B10`** — it no longer gates A7/A8; `FULL_EXECUTION_PLAN.md` updated (dependency map, A7 status line, Track B section + recommended-order note, immediate-next-actions row). **Then, same session: B1 flagged 🔴 open — defer to CEO at A7.** While explaining B1 (Timesheet → Central Hub) in detail, the user pushed back: their real billing process is time × rate per employee + other additional (expenses) → client invoice, with **no overtime** — a mismatch with B1's attendance/payroll framing (Work Location, Clock In/Out, Overtime, late flags). Checked the actual code before agreeing: `js/pages/reports.js` already computes `hours × billable_rate` per project, live, with zero dependency on B1; there's no invoice-generation step anywhere (labor cost and expenses are two separate reports, combined by hand today); the only part of B1 arguably still worth keeping is locking hours once billed. **User's decision: don't resolve this now — raise it at A7 (team review) with the CEO, and do a full Track B review there too**, not just B1, since the same reality-check might apply to other roadmap items. `FULL_EXECUTION_PLAN.md` updated: A7's agenda formalized into 2 parts (functional/UX feedback; full Track B roadmap review with CEO — exit is a revised Track B to build against); B1 got a detailed 🔍 open-question callout (what's already built, what's missing, 3 options for the CEO) + a new **D0** decision-table row + its build-phase section marked as "if option (a) is chosen" so it can't be mistaken for a decided plan. Docs only, no cache change.)*
> *(Revised: 2026-07-11 (e) — ✅ **Team-page visibility scoping — ALL 4 verification legs green + 1 real pre-existing bug found & fixed. Ready to merge v=125 to main, pending explicit go-ahead.** The manager→project-client positive case (last open item) is now confirmed: assigned David Bowman to Delos's "Mesa Hub Maintenance Operations" project → `team_scope_probe` as him now shows **8 rows (6 staff, same as before + 2 client rows)**, both `client_id=c0000001-...-007` (Delos) — exactly matching, proving `is_client_on_my_projects()` correct end to end. **🐛 Real bug found along the way (unrelated to this feature's own code, pre-existing):** `project_assignments` had been completely unwritable since it was first created — a trigger (`trg_project_assignment_role`) calling `check_assignment_role()`, a function written only for `task_assignments`' shape (`NEW.assignee_type`, a column `project_assignments` doesn't have) was mistakenly also attached to `project_assignments`. Every INSERT/UPDATE on that table has always thrown `record "new" has no field "assignee_type"` — invisible until now because nothing ever wrote to it before this round's Managers UI. **Fixed in `20260713b_fix_project_assignments_trigger.sql`** (✅ applied + verified — the trigger now only exists on `task_assignments`): drops the misattached trigger from `project_assignments` only; role correctness there is already fully enforced by the existing `pa_insert` RLS policy, so no replacement check was needed. **Full verification summary:** ✅ migration `20260713_team_visibility_scoping.sql` confirmed live (`pg_policies` dump shows the scoped `qual`); ✅ `anon_probe.ps1` 61/61; ✅ `f01_prod_client_probe.ps1` 41/41, 0 FAIL (CLIENT-01 intact); ✅ `team_scope_probe.ps1` as member — 0 clients, correct same-group subset (cross-checked exactly against the real group roster); ✅ same probe as manager, unassigned baseline — 0 clients (correct negative case); ✅ same probe as manager, assigned — 2 clients appear (correct positive case). **Next: user's explicit go-ahead to merge `claude/a2-a3-continuation-str4yq` → `main` and deploy v=125.** Test artifact left in place: David Bowman is now genuinely assigned as manager on Mesa Hub Maintenance Operations in `project_assignments` — this is real, not test-only, data; leave as-is unless the user wants it removed.)*
> *(Revised: 2026-07-11 (d) — 🔐 **Team-page visibility scoping — migration CONFIRMED APPLIED + LIVE in prod; 3 of 4 verification legs green, 1 still open.** `20260713_team_visibility_scoping.sql` is live — the `profiles_select` policy `qual` was dumped directly from `pg_policies` and shows the new scoped predicate (`is_admin()` / `shares_group(id)` / `is_my_report(id)` / `is_client_on_my_projects(id)`), not the old blanket clause. **Verified green:** ✅ `anon_probe.ps1` 61/61 PASS (RLS change opened no anon leak); ✅ `f01_prod_client_probe.ps1` 41/41 PASS 0 FAIL against the real Delos client login (`profiles → 1 row` — CLIENT-01 isolation intact through the change); ✅ `team_scope_probe.ps1` as **member** (`03-1-014-79`) — role resolved correctly, 0 client rows, 6 staff rows; ✅ same probe as **manager** (`02-3-003-56`, David Bowman) — same 6 staff rows, 0 client rows. The "6 rows" result was cross-checked against the real Teams-page group roster from this session's screenshot (union of "ME Team #1" + "Mechanical Engineering" = exactly David Bowman/Evelyn Wang/Hikaru Sulu/Paul Atreides/Takeshi Kovacs/Jean-Luc Picard; James Kirk, in no group, is correctly the one excluded) — an exact match, not a guess. **🟡 Still open — the manager→project-client positive case is unconfirmed:** after assigning David Bowman to a Delos project via the new Projects → Assign → Managers control, the manager probe still shows 0 client rows (same as the unassigned baseline) — either the assignment didn't persist, or `project_assignments` never had a PERMISSIVE write policy for admin (the table was dormant/unused by the app until this round, so its RLS may only ever have had the `client_block_*` RESTRICTIVE policy, no admin INSERT/UPDATE grant). Diagnostic SQL handed to user (`SELECT * FROM project_assignments;` + `SELECT policyname,... FROM pg_policies WHERE tablename='project_assignments'`) — result not yet returned. **Do not merge v=125 to main until this is resolved** (if it's a missing policy, needs a small follow-up migration granting admin/owner ALL on `project_assignments`, matching every other admin-managed table's pattern). Everything remains on `claude/a2-a3-continuation-str4yq`, unmerged.)*
> *(Revised: 2026-07-11 (c) — 🔐 **Team-page visibility scoping built (cache JS v=124→v=125) — ⚠️ migration NOT yet applied; deploy AFTER Studio apply.** New feature from the A4 walkthrough finding (a member saw the whole org roster). Per-role scoping, enforced in RLS (real boundary, not just UI): **member** = self + same-group staff only (no clients); **manager** = same-group staff + direct reports + read-only client accounts on their assigned projects; **admin/owner** = all staff + all clients (read-only); **client** = own row only (UNCHANGED — clients stay portal-only; deliberately did NOT add "see same-company clients" because it has no surface and would break the 41/41 f01 probe's `profiles=1 row`). New migration **`20260713_team_visibility_scoping.sql`** (⚠️ **pending Studio apply**): SECURITY DEFINER helpers `shares_group()`/`is_my_report()`/`is_client_on_my_projects()` (search_path pinned, anon EXECUTE revoked) + replaces the blanket `profiles_select` policy with the role-scoped predicate. Live schema surfaced via Studio first (profiles policy + `get_my_role`/`is_admin`/`is_manager_of`/`my_client_id` bodies + column shapes) — `is_manager_of` keys on `employees.id` so a new `is_my_report()` (keyed on user ids) was written for the policy. **Requester-name cross-group risk turned out moot**: the request tables are SELECT-visible only to the requester or `is_admin()`, so no manager-approval name blanks. App: `getUsers(isAdmin,{includeClients})` (Team passes true; Reports/Projects keep clients out); `team.js` renders client rows **read-only** (no rate/role/group/delete); **Projects assign modal gained a Managers section** (writes `project_assignments` via the previously-dormant `assignManager`/`unassignManager` + new `getProjectManagers`) so manager→project-client visibility has real data; **reports.js** now fetches billable_rate only for admins (`if (showAmount)`) — was pulling rates into managers' payloads. New probe `team_scope_probe.ps1` (member/manager profiles-scoping). RBAC matrix §5 updated. `check:parse` 56/56. **⚠️ DEPLOY ORDER (critical):** apply the migration in Studio + re-run anon (61/61) + f01 client (41/41) probes + `team_scope_probe.ps1` as member & manager, THEN merge v=125 to main — pushing the frontend first (blanket RLS still live) would briefly show members ALL staff+clients on Team. Committed to `claude/a2-a3-continuation-str4yq`, not merged. A4 audit otherwise still mid-run (1A/1B/1C/1D done; 1E/1F/1G/1H/2/3/4 pending).)*
> *(Revised: 2026-07-11 (b) — 🔍 **A4 audit running live (user-driven) — 4 phases green + 1 real finding fixed. Cache JS v=123→v=124.** Progress: **1A anon probe 61/61 PASS** (via the new `anon_probe.ps1`), **1D client probe 41/41 PASS 0 FAIL** (real `role='client'` Delos login), **1B member 5/5 PASS**, **1C manager**: team-scoping + `#employees`-bounce pass; billable-rate-masked-for-non-admins confirmed pass (managers/members see "—", only owner/admin see values); 2-tier + single-tier leave approval **deferred to Phase 2** (no pending report requests existed to act on). Also confirmed **not** bugs: manager reaching `#clients`/`#reports` but bouncing `#employees`/`#admin-logs` is correct per the `routeAllowed` matrix; a member seeing the whole org on the **Team** page is **by design** (RBAC §5 = Team is a staff-wide view-only directory, no rate). **🐛 Finding A4-F1 (Low, fixed): `getUsers()` returned `role='client'` profiles**, so external portal-only client accounts (Delos.Test1/2) leaked into the **Team directory** (visible to members → exposed client emails), the **Reports** user list, and the **Projects** member-assignment picker; an admin could even set a billable rate / change the role of a client from Team. Root-fixed by excluding `role='client'` at the source in `js/api/users.js` (`.neq('role','client')` — fixes all three surfaces) + dropped `'client'` from `js/pages/team.js` `ROLES` (so staff can't be converted to client via the inline role dropdown, and the now-empty filter option is gone). `check:parse` 56/56. **Data note (not code):** `testclient@example.com` is mis-roled as `member` in prod — stray test account, delete it or it's wiped at the A8 roster swap. **▶ Still to run:** 1E (`edge_probe.ps1`), 1F/1G/1H/3 (Studio SQL), Phase 2 (browser walkthrough incl. the deferred leave-approval flow), Phase 4 (UI/UX + CSP console check).)*
> *(Revised: 2026-07-11 — 🔧 **A4 in progress (user running it live): `anon_probe.scratch.ps1` found to never have existed.** User started running Phase 1A of the execution packet on their own machine and hit `CommandNotFoundException` — the script had been referenced as "kept locally, gitignored" across many rounds (R39 onward) but a fresh search of this checkout (and every prior one, per repeated greps this session) turned up neither the script nor its source doc `AUDIT_2026-06-11_GOLIVE.md` anywhere. That "45/45" / "~56" target had been pointing at nothing real for a long time. **Fixed at the root, not just documented around:** wrote a real, repo-tracked `anon_probe.ps1` (repo root, not gitignored — uses only the public anon/publishable key, same one already shipped in the frontend, so there's no secret to protect). Its table/RPC list is **derived directly from the app's own code** (`grep -rohE ".from\('[a-z_]+'\)" js/` → 45 tables, `grep -rohE ".rpc\('[a-z_]+'" js/` → 11 RPCs) plus 4 known schema objects the client never queries directly but that must still deny anon (`pn_counters`, `login_attempts`, `pn_item_snapshot`, `pn_render_template` — server-side-only, targeted by the A1 hardening migration). **Real target: 61/61 PASS** (47 tables + 1 dropped-view regression check + 13 RPCs) — supersedes the old "~56" guess, which itself was never grounded in an actual derived count. Updated `PRE_LAUNCH_AUDIT_PLAN.md` (1A section + pass-criteria row), `PRE_LAUNCH_AUDIT_EXECUTION_PACKET.md` (1A section), and `FULL_EXECUTION_PLAN.md` (A3.6) to point at the real script and real number. **User still needs to actually run `./anon_probe.ps1`** and report the result — this round only unblocked the step, it didn't complete it. A4 otherwise continues per the full-steps walkthrough already given. No code/cache change (new script file only).)*
> *(Revised: 2026-07-10 (c) — ✅ **Round 60 cont.: A6.3 done — structure-doc nav re-sync.** `HE_interactive_timesheet_plan.md` §7 (File Structure, frozen at R35) and §10 (Sidebar Navigation, frozen at the pre-WMS core-tracker nav) plus `HE_WMS_Specification.md` §14 (Nav Map, missing Client Portal/Admin Logs/Part Numbers, and still listing a never-built "Applicants" placeholder) all re-verified directly against `app.html`'s sidebar markup + real `js/pages`/`js/api`/`js/components` directory listings — not carried forward from earlier rounds. All three now show the full **19-route nav** (`#tracker` … `#help`), the `app-init.js`/`login-init.js` R50 externalization, the R43 `holidays-*`/`expenses-*` submodule splits (exact filenames, not the slightly-wrong names an earlier round's log entry used), `auditLog.js`/`partNumbers.js` (api), and correctly note that recent migration `.sql` files are tracked at repo root while `supabase/` (functions/schema/backups/seeds) stays gitignored except the client-probe `.ps1`. `FULL_EXECUTION_PLAN.md` A6 flipped to 🟡 partial (A6.3 ✅; A6.1 template wording + A6.2 leave-policy decision remain 🔴, both blocked on a 🧑 decision — not something a session in this container can resolve alone). No code/cache change. **▶ Next:** A4 (execution packet against prod, needs human/network) and A6.1/A6.2 (need 🧑 input) are the only launch-path items left; both require the user.)*
> *(Revised: 2026-07-10 (b) — ✅ **Round 60 cont.: A5 done — Help page refreshed, cache JS v=122→v=123.** `js/pages/help.js` was frozen since Round 42 with zero coverage of Part Numbers, the 2-tier leave flow, Client Portal, or Admin Logs. Added bilingual (EN/TH) content: **User Guide** — Part Numbers (format, minting on `#part-numbers`, revision bump + Compare), two-step leave approval (what `manager_approved` means for the employee, no action needed). **Admin Guide** — Manage client logins (provision/reset/delete + Copy credentials), Client Portal (aggregated hours, masked expenses, never employee names), Part Numbers governance (Categories/Lists managers, client+project code prerequisites, customer-PN mode), HR approval step (AWAITING HR APPROVAL + HR Approve button), new **Admin Logs** section (entity/actor/date filters, pagination). Every factual claim (button labels, filter fields, page-size behavior) independently verified against the real source (`clients.js`, `holidays-approvals.js`, `adminLogs.js`) before commit — not just trusted from the build. Account Status tab was already covered pre-existing, so no change needed there. `npm run check:parse` 56/56 clean. **🟡 Still needed: 🧑 in-browser spot-check in prod** — this container can't render pages to verify visually. **▶ Next: A4** (execution packet against prod, needs human/network) remains the only launch-path item not yet closeable from this container; **A6** closeout content (template wording, leave-policy decision) is the other parallel-any-time item.)*
> *(Revised: 2026-07-10 — ✅ **Round 60: A2 + A3 done (docs + probe scripts only, no cache change).** Track A tasks A2 (11 doc-accuracy corrections) + A3 (audit coverage extensions) both closed and flipped to ✅ in `FULL_EXECUTION_PLAN.md`. **A2:** dead `he-cells.github.io` URLs → `surasaknie.github.io` and stale `22/22`/`45/45` probe targets fixed across `PRE_LAUNCH_AUDIT_PLAN.md` + `PRE_LAUNCH_AUDIT_EXECUTION_PACKET.md`; roadmap Part-2 status table re-synced to the Master Plan (Phases 0/1/6/7 all ✅, only Phase 2/M1 stays 📋); OTP/freeze auth design marked **superseded** by the shipped Employee-ID+password model; never-created `WMS-expansion-roadmap.md` reference dropped; CLIENT-01 archived as ✅ shipped R38–39 and the Future-roadmap section pointed at Track B. **A3:** CORS regression curl block (7 Edge Fns × new/old origin) appended to packet Phase 1E — catches the R53 outage class the input-validation block can't; Part Numbers policy SQL (**expect 16 policies**, `pn_items` INSERT positive-control = 0) added to plan+packet Phase 1F; **new Phase 2H** Part Numbers walkthrough in the plan + a Part Numbers bullet in the packet Phase 2; P1–P4 Part Numbers integrity queries added to plan+packet Phase 3; **both probe scripts** (`f01_prod_client_probe.sh` + `supabase/probes/f01_prod_client_probe.ps1`) extended with 6 `pn_*` `check_zero` + a `pn_items` write-denied check → **41 checks total** (34 R59 baseline + 7; verified against the real pn schema — `part_number`/`cat_code`/`seq`, `pn_counters.scope`/`last_seq`; `bash -n` clean); anon-probe re-baseline note (~56) added to packet Phase 1A. All SQL cross-checked against the actual `20260710`/`20260711` migrations (16 pn policies confirmed: 10 explicit + 6 `client_block_*`; no INSERT policy on `pn_items`). **▶ Next: A4** — a human/network run of the execution packet against prod (targets are now current); parallel any time: A5 Help-page rebuild (needs cache bump, next is v=123) + A6 closeout content. Cache still v=122.)*
> *(Revised: 2026-07-09 (g) — ✅ **R59 cont.: A1 verified applied — Track A tasks A0 + A1 now BOTH fully complete.** `20260709_lint_search_path_and_execute_hardening.sql` confirmed live: all 3 footer VERIFY queries, **run individually** (Studio only shows the last statement's grid when several are pasted together, so they were split one-per-tab to remove ambiguity), returned 0 rows — (2a) no fn missing a `search_path` pin, (2b) no SECURITY DEFINER fn anon-executable, (2c) the 8 trigger fns no longer grant EXECUTE to `authenticated`. The residual 0029 linter WARNs on the 9 RPCs + 10 RLS helpers + 4 pn fns are **by design — do not re-flag** (they must stay authenticated-executable). Migrations table + `FULL_EXECUTION_PLAN.md` A1 flipped to ✅. **All three A0/A1 Studio migrations (`20260712`, `20260712b`, `20260709`) are now applied + verified in prod.** **▶ Next: A2 + A3** (🤖 — audit-plan/packet doc corrections + audit coverage extensions: CORS regression curl block, Part Numbers policy SQL/2H walkthrough/integrity queries, probe-script pn checks) then A4 (run the audit against prod). Cache still v=122, no code change this round.)*
> *(Revised: 2026-07-09 (f) — ✅ **R59 cont.: F-01-class gap fully closed — client probe re-run PASSED 34/34, 0 FAIL.** After the "Reset pw" hiccup resolved itself with a hard refresh (stale session token, not a code bug), the probe was re-run against a genuine `role='client'` login (`Delos.Test1@example.mail`, Delos Incorporated) instead of the earlier manager account. Clean result: own-scope profile/company/project access correct (1 profile, 1 client, 3 projects via `get_client_project_summary`), and **all 23 internal/PII tables — including all 12 newly-blocked ones from today's `20260712_client_block_expanded.sql`** (`employee_compensation`, `evaluations`, `employee_documents`, `employee_audit_log`, `leave_balances`, `flex_holiday_swaps`, jtcr/ncr/deletion requests, `travel_claims`, `project_assignments`, `groups`) **— return 0 rows for a real client session**, plus all writes denied. This closes the loop on the entire PR #26 salvage thread end-to-end: orphaned draft PR (2026-07-03) → landed on main (R57) → migrations applied + verified via SQL (R59) → **now proven live via authenticated client probe.** **Only A1 remains open** (`20260709_lint_search_path_and_execute_hardening.sql` — instructions given, run/VERIFY not yet confirmed).)*
> *(Revised: 2026-07-09 (e) — 🔎 **Round 59: A0 migrations applied + verified live in prod; A1 + client-probe re-run paused mid-session — pick up here.** No cache change. **✅ Confirmed applied** (user ran both in Studio, results checked against live queries, not just the "Success" message — institutional caution from the BEGIN/COMMIT history paid off again): `20260712_client_block_expanded.sql` — `pg_policies` query returned 29 `client_block_%` rows total (the 12 new ones targeted + 11 from `20260708` + 6 pre-existing on `pn_*` tables), and `audit_log_select_admin.qual` confirmed `is_admin()`; `20260712b_f05_rpc_search_path.sql` — `pg_proc.proconfig` confirmed `search_path=public, pg_temp` on all 3 F-05 RPCs. **⚠️ NOT confirmed:** `20260709_lint_search_path_and_execute_hardening.sql` (A1) — instructions were given in the same sitting but the session moved on before a run/VERIFY result came back; treat as unapplied until re-confirmed. **🔎 Diagnostic finding, not a regression:** the client-probe re-run (`f01_prod_client_probe.ps1`) was first run against `hubbleengineering@gmail.com` — the long-standing test manager account "David Bowman" (`role='manager'`) — producing a scary-looking **17 PASS / 17 FAIL**. Root-caused as a **test-methodology error, not an RLS failure**: `client_block_*` RESTRICTIVE policies key off `auth_is_client()`, which is correctly `false` for a manager, so full team-data visibility is expected/correct, and `get_client_project_summary → 400` is likewise expected for a non-client caller. **New standing lesson: never run the F-01 probe against an admin/manager login** — it will always produce this same false-positive pattern; a genuine `role='client'` login is required. Found **2 existing client logins under "Delos Incorporated"** (`profiles.id` `cb45d583-f743-4d14-9399-21d863c84c1e` and `bcdaa08b-d64d-4fcb-963c-d1849783143d`) — passwords unknown/unrecoverable, next step is **Clients page → Delos Incorporated → Manage logins → Reset pw** on either, then re-run the probe with the resulting credentials (target 0 FAIL, ~35 checks). **Session paused here at user request** — resume with: reset Delos password → re-run probe → paste + run A1 + its 3 VERIFY queries → then flip A1 to ✅ and move to A2/A3.)*
> *(Revised: 2026-07-09 (d) — 🖱️ **Round 58: modal backdrop-click-to-close removed app-wide. Cache JS v=121→v=122. MERGED to main.** User-reported bug fixed before A0: accidentally clicking outside a modal (e.g. Profile) was closing it. Surveyed every `.modal-backdrop` in the codebase — **35 close-on-backdrop-click sites across 16 files** (`confirmModal`/`promptModal`, `entryModal`, `prefsModal`, `profileModal`, `clients`, `documents`, `employees`, `evaluation`, `expenses-approvals`, `expenses-report`, `holidays-approvals`, `holidays-holidays`, `partNumbers`, `projects`, `tags`, `team`) — and removed the `if (e.target === backdrop) close()`-style listener from every one. **Esc preserved on purpose** (not part of the complaint, and a standard dialog-dismiss convention): each site now sets `backdrop._escClose = close` where the listener used to be; the global handler in `js/app-init.js` was changed from synthesizing a `.click()` on the topmost backdrop (which would have kept firing the old close-on-click logic — same code path as a real click) to calling `_escClose?.()` directly. **Verified in a real headless-Chromium/Playwright run** against the actual shipped `confirmModal.js` + the literal new Esc-handler snippet (harness built + torn down in-session, never committed): backdrop click leaves the modal open, ✕/Cancel/Confirm/Escape all still resolve correctly, and the modal stays fully functional after an ignored backdrop click. `npm run check:parse` 56/56. Submodule pins bumped 121→122 in `holidays.js`/`expenses.js` (4 submodules changed). CLAUDE.md's Modal Pattern section rewritten to document `_escClose` as the required convention going forward — **no future modal should have a backdrop-click-close listener.** No DB/migration impact — pure client-side. Does not block or interact with plan tasks A0/A1 (Studio migrations) — those remain the next action.)*
> *(Revised: 2026-07-09 (c) — 🛟 **Round 57: PR #26 salvage — orphaned 2026-07-03 audit recovered onto main. Cache JS v=120→v=121 / CSS v=39→v=40. MERGED to main.** Branch-cleanup review found draft **PR #26** (`claude/next-session-plan-h9psa4`, a parallel full audit from 2026-07-03 that called itself "Round 54" before the docs' R54 existed) open, unmerged, and tracked **nowhere** — its full report was gitignored/local-only, so the PR body is the only surviving record. Verified its fixes were genuinely missing from main, then salvaged: **(1) Migrations renamed** (branch's 20260710/20260711 dates were since taken by Part Numbers) → **`20260712_client_block_expanded.sql`** (RESTRICTIVE `client_block_*` on 12 missed tables — worst: `employee_compensation` salary PII, which the app reads at `js/api/employees.js:210` while the F-01 probe checked the wrong name `compensation_records`; + `audit_log_select_admin` off `get_my_role()` onto `is_admin()`) + **`20260712b_f05_rpc_search_path.sql`** (pins `search_path=public, pg_temp` on the 3 F-05 RPCs); both wrapper-stripped, **⚠️ pending Studio apply = plan task A0** (run with A1). **(2) Probe scripts extended** (.sh + `supabase/probes/.ps1`): `compensation_records`→`employee_compensation` + 11 new tables (~35 checks); stale root duplicate `.ps1` deleted (branch's consolidation). **(3) Code fixes re-applied** (25 files wholesale where main was untouched since base; hand-ported `projects.js`/`clients.js`): XSS `safeColor()` on project-dot color in `projects.js`+`tracker.js`; **stale submodule `?v=` pins** in `holidays.js`(104)/`expenses.js`(103/106) → 121 (returning browsers could hold stale submodule code across deploys — new CLAUDE.md rule); `.ts-remove-btn` `opacity:0`→0.6 (visibility rule); approval **prior-status guards** in `api/expenses.js`+`leaves.js` (retry can't re-fire ledger trigger) + new **`overrideTravelClaimStatus`** (override no longer coerced to `manager_approved` via reused approve fn); `confirmModal` bespoke Esc handler removed (house pattern); `clients.js` delete-login fetch now checks response (was silently deleting the profile row even when the Edge Fn failed); `copyLastWeek` ICT date fix in `api/timeEntries.js`; login 401/429/500 messaging (`login-init.js?v=2`); doc-merge `sanitizeHtml` gap; misc attr escaping (`tags/team/dashboard/documents/prefsModal/profileModal/expenses-*/holidays-*`). PR #26's stale doc edits (incl. `NEXT_SESSION_PLAN.md`) deliberately NOT ported — superseded by `FULL_EXECUTION_PLAN.md` (which gained task **A0**). `npm run check:parse` 56/56 clean. **PR #26 closed as salvaged; branch cleanup can proceed (7 squash-merged branches + h9psa4 once user confirms).** ⚠️ Live spot-checks after deploy: money approve/reject + travel-claim override; Esc on confirm dialogs; Projects color swatches; Timesheet ✕ visible.)*
> *(Revised: 2026-07-09 (b) — 📋 **R56 cont.: [FULL_EXECUTION_PLAN.md](FULL_EXECUTION_PLAN.md) created** — consolidated, detailed plan for ALL remaining work after two review passes found the audit plan/packet stale (dead `he-cells` URL, probe targets 22/22→23/23, anon 45/45 predates audit_log+pn tables, no CORS test despite the R53 outage, no Part Numbers coverage in 1F/2/3) and the roadmap doc staler (Phases 0/1/6/7 all shipped but still listed unbuilt; auth section describes the superseded OTP/freeze design; CLIENT-01 still listed as "future" though live since R39). **Track A launch path:** A1 apply `20260709` lint migration (user, Studio) → A2 eleven doc corrections → A3 audit-coverage extensions (CORS curl block, pn policy SQL + NO-INSERT positive control, new Phase 2H walkthrough, 4 pn integrity queries, probe-script pn checks → ~30, anon re-baseline → ~56) → A4 run audit (user/network) → A5 Help-page refresh (v=121) → A6 closeout (template wording, leave-policy decision, structure resync) → A7 team review → A8 roster swap (RSK-0). **Track B post-launch:** B1 M1 Central Hub (5 design decisions D1–D5 + build phases a–e), B2 daily Sheets export, B3 backlog batch (L-ADMCK/L-INITYR/RLS-M2/M3/F-09 etc.), B4 dashboard toggles, B5 BOM design, B6 deferred features (⚠️ receipt upload would break the "no-Storage = complete DB backup" assumption), B7 decision queue (role model / leave carry-over / FX source), B8 Supabase-Pro-first paid-tier rec, B9 verify-and-close (client-provision audit row likely already closed by R45 — `clients.js:599`). Master Plan gained a pointer + sub-doc index row. Docs only, no cache change.)*
> *(Revised: 2026-07-09 — 📋 **Round 56: docs-only session, no cache change.** (1) **Google Sheets export cadence changed weekly→daily** per user request in `HE_Integrations_and_WMS_Roadmap.md` Part 1 (title, trigger design, `weeklyExport()`→`dailyExport()`, `setUpTrigger()` now `everyDays(1)`, cost analysis notes the ~7× read-volume increase — still $0/trivial); cross-refs updated in `Timesheet_WMS_Master_Plan.md`, `HE_interactive_timesheet_plan.md`, `HE_WMS_Specification.md`. Still ⏸ parked (blocked on app-live + user-supplied Sheet URL) — plan-only, nothing built. (2) **Phase 2 (M1 Timesheet → Central Hub) roadmap reviewed with user** — no doc changes, informational (scope: attendance fields, auto-fill from Leave/Holiday/Trip, weekly submit→approve→lock, project cost tracking; detail in `HE_WMS_Specification.md` §4). (3) **`20260709_lint_search_path_and_execute_hardening.sql` stripped of its `BEGIN;`/`COMMIT;` wrapper** — same fix pattern as the two Part Number migrations (R54/R55), which twice silently ran only a fragment in the Supabase SQL Editor when transaction-wrapped; each `DO` block autocommits independently so this is still safe/idempotent. ⚠️ **Still NOT applied** — this container re-confirmed a hard 403 at the proxy to `sjkggguedgtynktymzes.supabase.co`, so Studio apply remains a user action. (4) **New go-live gate added:** refresh `js/pages/help.js` before team review — grepped the file, confirmed **0** mentions of CLIENT-01/Client Portal, Admin Logs, Account Status tab, or Part Numbers (it still only reflects Round 42 content); added as execution-order step 3 in `PRE_LAUNCH_AUDIT_PLAN.md` (renumbering team review→4, sign-off→5), the Closeout row here, and the Master Plan's next-priority pointer. No files/pages/migrations touched other than the one SQL file; no `V` bump.)*
> *(Revised: 2026-07-08 — ✅ **Round 55: Part Numbers v2 (spec-driven) — baseline JS v=116→v=120, MERGED to main + deployed.** Reworked to `PART_NUMBERING_SPEC.md`: format `CC-PPP-AA-BBB`→**`CCC-PPP-CAT-SEQ`** (CAT = 3-letter governed code; 11 seeds ASM/PCA/PCB/CBL/ELC/PRT/OTS/FMW/DOC/PKG/TOL with "covers" help + decision ladder). **Retired `pn_projects`** → part numbers hang off the real `projects`/`clients` tables via new 3-char `code` columns (CCC on client, PPP on project). New `pn_attributes` (material/finish/vendor/fab_process/color, admin/manager-managed) + item FK columns; `pn_project_config` (per-project customer-PN mode/template); per-revision `snapshot jsonb` powering a new ⓘ info/**Compare** modal (two revisions side-by-side). Rewrote `pn_create_item`/`pn_bump_revision` + new `pn_item_snapshot`. Clients + Projects pages gained code inputs; Projects rows got a Part Numbers deep-link (`#part-numbers?project=<id>`). New migration **`20260711_part_numbers_v2.sql`** **✅ applied in prod Studio 2026-07-08** (11 letter codes verified; no BEGIN/COMMIT wrapper — the wrapped v1 file ran only a fragment in the SQL Editor; scratch-PG16 verified: v1→v2 transform idempotent + 12 behavioral/RLS tests). **Post-launch tweaks (v=117→v=120):** attributes default to **TBD** when unset; **client filter** narrows the project picker; fixed the **Lists** button (click event was passed as the attribute `kind` → modal never opened). Cache **v=116→v=120**; **merged to `main` (fast-forward) + deployed 2026-07-08.** ⚠️ **GitHub Pages gotcha:** setting the repo **private** unpublished the Pages site (Free-plan Pages need a public repo), and going public again does **not** auto-re-enable Pages — re-set Source in Settings→Pages. Keep the repo **public** (app-only; secrets gitignored; security = Supabase RLS + Edge-Fn CORS, and the published site is public by URL regardless). Branch `claude/part-number-generation-7rtbht` (restarted from main after v1 PR #27/#28 merged). Note: `DRONEKYLL_PART_NUMBERING.md` is a separate program-scoped scheme, not built into the WMS.)*
> *(Revised: 2026-07-07 — ✅ **Round 54: Part Number Generator shipped — baseline JS v=115→v=116.** New `#part-numbers` WMS page: generate part numbers in the house format `CC-PPP-AA-BBB` (CC company abbr, PPP project abbr, AA 2-digit type code, BBB per-(project,type) increment). Own project registry (`pn_projects`, independent of timesheet projects), editable AA type-code list (`pn_type_codes`, 7 seeds), items minted ONLY via the `pn_create_item` SECURITY DEFINER RPC (atomic `pn_counters` upsert → gap-free, deleted numbers never reused; **no INSERT policy on `pn_items`**), dual part numbers (internal always + optional customer PN via `pn_render_template` template placeholders or manual entry with case-insensitive per-project unique index), `pn_bump_revision` RPC + `pn_item_revisions` history, immutability guard trigger. New files: `20260710_part_numbers.sql`, `js/api/partNumbers.js`, `js/pages/partNumbers.js`; nav + `wmsRoutes` + `pages` map wired in `app.html`/`app-init.js`. Migration **✅ applied in prod Studio 2026-07-07** (7 type-code seeds verified) — ⚠️ **had to apply the autocommit variant: the `BEGIN…COMMIT`-wrapped file silently ran only a fragment in the Supabase SQL Editor ("Success, no rows" but zero objects created); strip the transaction wrapper if a wrapped migration ever "succeeds" but leaves nothing behind.** Commits `5dec214` + `2776926` (branch `claude/part-number-generation-7rtbht`, no PR yet). 🟡 **KNOWN FOLLOW-UP — PN v2: the AA type-code scheme needs rework.** Current model is a flat editable code+description list (00 Assembly / 01 Part / 02 Purchased / 03 Electrical / 04 Pneumatic-Hydraulic / 05 Fastener / 09 Document) with no rule for *which* code a new item gets — user flagged this as "still not that great." Rework the AA taxonomy/assignment logic next session. Longer-term direction remains BOM management (schema already BOM-ready: uuid-PK `pn_items`).)*
> *(Revised: 2026-07-02 — 📋 **Round 52: GitHub account transfer prep — baseline JS v=115.** The owner is transferring BOTH repos — `hubble-wms` (app + Pages) and `hubble-wms-backups` (nightly encrypted DB dumps) — to a new GitHub account; **Supabase stays** (`sjkggguedgtynktymzes`). Full audit result: the app code is transfer-safe (relative asset paths; dynamic OAuth `redirectTo` in `auth.js`; CSP has no `github.io` entry — `'self'` covers any Pages origin; probe scripts + CI reference only Supabase). New **[REPO_TRANSFER_CHECKLIST.md](REPO_TRANSFER_CHECKLIST.md)** captures the required post-transfer steps, the critical ones being: **Supabase Auth URL Configuration** (Site URL + Redirect URLs → new Pages origin; Pages URLs do NOT redirect — miss this and Google login falls back to the dev Site URL, the R24-03 failure), verify Google OAuth JS origins, check Edge Fns for origin allow-lists, confirm Pages re-enabled, and on the **backups repo**: repo-level secret `SUPABASE_DB_URL` + var `AGE_PUBLIC_KEY` transfer automatically, but confirm the `nightly-db-backup` cron didn't stall (push a commit / manual run) and re-Watch from the new account. Also shipped: **team.js App-link subpath fix** — Add-Member modal's link was `origin + '/index.html'` (drops the `/hubble-wms/` Pages subpath → 404); now `new URL('index.html', window.location.href).href` (same idiom as `auth.js`). Cache **v=114→v=115**. Docs updated with `<new-account>` placeholders — replace them with the real account name after the transfer. Note: R51 (2026-07-01, PRs #22/#23) — dropped SECURITY DEFINER view `client_project_totals` + DB linter hardening — was recorded in CLAUDE.md only; see those PRs.)*
> *(Revised: 2026-07-01 — ✅ **Round 50: Pre-launch audit Phase 5 must-fix items closed — baseline JS v=114.** **CONV-M4 verified resolved** — grepped `js/`, `app.html`, `index.html`: zero `3030` occurrences in shipped code; only in docs referencing the dev preview server, no code change needed. **L-CSP fixed**: added `<meta http-equiv="Content-Security-Policy">` to both `app.html` and `index.html`. Caught + fixed 2 bugs in the plan's draft policy before shipping: (1) missing `fonts.gstatic.com` in `font-src` (Inter font load would've silently broken); (2) `script-src 'self'` would have silently blocked both files' inline `<script type="module">` blocks — fixed by **externalizing** them into new `js/app-init.js` (from `app.html`) and `js/login-init.js` (from `index.html`), each verified byte-for-byte identical to the original inline content via diff except for the necessarily-adjusted `./js/...`→`./...` import paths (file moved into `js/`) and the `V` cache-bump. Kept `script-src` strict (no `'unsafe-inline'`) as a result; `style-src` still needs it (many inline `style="..."` attributes, out of scope). Added `frame-ancestors 'none'` beyond the plan's literal draft. Cache **v=113→v=114**; `npm run check:parse` 54/54 files clean. **⚠️ Not live-verified**: this Claude Code container gets a hard 403 policy denial reaching prod Supabase and GitHub Pages (confirmed via the proxy status endpoint) — so Phases 1–4 of the audit (Studio SQL, curl against Edge Functions, live-browser role probes and walkthroughs) cannot run from this container at all. Assembled [PRE_LAUNCH_AUDIT_EXECUTION_PACKET.md](PRE_LAUNCH_AUDIT_EXECUTION_PACKET.md) with ready-to-run SQL/curl/checklists for those phases, plus a post-push spot-check for 0 CSP console violations. **NEXT: push this batch → run the execution packet against prod → team review → roster swap.**)*
...
| 5 | **Closeout** | partial | Template wording (user supplies final HE text → TEMPLATES editor; all 9 templates are placeholders) · leave pro-rating decision recorded (Dec-31 reset policy already noted) · ✅ **bilingual Help page done 2026-07-10 (R60 cont., cache v=123)** — now covers Client Portal, Admin Logs, Account Status tab, and Part Numbers in EN+TH (🟡 in-browser prod spot-check still pending) · **re-sync the structure docs** (timesheet plan §7 + Spec §14 nav tree) to final state at completion |
...
**Client Portal** (`js/pages/clientPortal.js`) scopes a client session to **only its own**
projects / summary hours / masked expenses. RLS isolation proven by the F-01 client probe
(**34/34, 0 FAIL** on a real `role='client'` login, R59) and hardened further by
`20260707` + `20260708` + `20260712` `client_block_*` policies. Nothing left open here.
