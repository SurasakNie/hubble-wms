...
| 5 | **Closeout** | pending | Template wording (user supplies final HE text → TEMPLATES editor; all 9 templates are placeholders) · leave pro-rating decision recorded (Dec-31 reset policy already noted) · **bilingual Help page LAST** (`js/pages/help.js`, EN+TH, role-aware, static — written after #3 so it documents the real login flow; this IS the user + admin manual) · **re-sync the structure docs** (timesheet plan §7 + Spec §14 nav tree) to final state at completion · 🟡 **added 2026-07-09 — refresh Help page before team review**: `js/pages/help.js` still reflects Round 42 content only — 0 mentions of CLIENT-01/Client Portal, Admin Logs, Account Status tab, or Part Numbers; must cover everything shipped since R42 before reviewers see it (see go-live sequence in the Master Plan) |
