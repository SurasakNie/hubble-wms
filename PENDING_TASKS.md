...
> *(Revised: 2026-07-03 — ✅ **Round 54: post-transfer full security/code audit + fixes — baseline JS v=116 / CSS v=40.** Ran the 4-dimension audit (XSS/RLS/correctness/conventions, 4 parallel `general-purpose` subagents) requested in `NEXT_SESSION_PLAN.md` item 1 — full report in `AUDIT_2026-07-03_FULL_PROJECT.md` (gitignored, local only). **All Critical + High findings fixed this round:** (1) **RLS Critical** — `20260708_client_block_internal_tables.sql`'s hand-maintained client-block array missed 12 internal tables (`employee_compensation` salary/rate PII being the worst miss, plus `evaluations`, `employee_documents`, `employee_audit_log`, `leave_balances`, `flex_holiday_swaps`, `job_title_change_requests`, `name_change_requests`, `deletion_requests`, `travel_claims`, `project_assignments`, `groups`) — new migration **`20260710_client_block_expanded.sql`** ⚠️ **PENDING prod Studio apply**, plus switched `audit_log_select_admin` off `get_my_role()` (the function root-caused for the original F-01 circular-RLS leak) onto `is_admin()`. F-01 probe scripts (both `.sh` and `.ps1`) extended to cover all 12 new tables; fixed a latent bug where the bash probe checked a nonexistent table name (`compensation_records` → `employee_compensation`); deleted a stale duplicate root-level `.ps1`. (2) **XSS High** — `projects.color` interpolated raw into a `style="background:"` attribute in `projects.js`/`tracker.js` with zero escaping (real attribute-breakout XSS) — fixed with `safeColor()`, matching the pattern already used correctly for tag colors nearby. Also fixed 4 XSS Mediums (dashboard.js/tags.js color-into-style missing `safeColor()`; documents.js generated-doc viewer missing the `sanitizeHtml()` layer its own preview already uses; profileModal.js initials unescaped) + several Low one-line escapes. (3) **Correctness High** — `copyLastWeek()` in `timeEntries.js` truncated dates via `.toISOString().split('T')[0]`, shifting by a full day for any ICT browser unconditionally (same anti-pattern as the historical `_datePlusDays()` bug) — confirmed dead/unwired code today (no UI ever calls it) but a landmine; fixed with `toISODate()`, same fix applied to `getExpiringDocuments()`. Also fixed a live Medium bug in `clients.js`'s "Delete client login" flow (discarded the `admin-set-account-active` fetch response before deleting the `profiles` row, so a failed deactivation still reported success) and a login-init.js Low (401/429/500 now distinguished instead of collapsing to one message). (4) **Conventions High ×3** — `holidays.js`/`expenses.js` tab-submodule `?v=` pins had drifted stale relative to their own content (a page-tab-level instance of the split-module-state bug the shared-module rule already guards against one directory up) — unified with the top-level `V`; `.ts-remove-btn` used `opacity:0` (unreachable on touch/keyboard, violating the documented action-button rule) — now visible at rest like `.tag-chip .remove-tag`; `.emp-select-wrap .form-input`'s `padding-right` was silently beaten by the global input denylist's higher-specificity shorthand (same bug class CLAUDE.md already documents for `.search-input`) — fixed with the same full-shorthand + `!important` pattern. Also removed `confirmModal`/`promptModal`'s bespoke Escape handler (contradicted the documented "no per-modal Esc handlers" rule; worked today only via fragile cross-file ordering). **Second pass (same round) — cleared most of the backlog:** RLS M-2 (f05 RPCs' `search_path` missing `pg_temp`) fixed via new **`20260711_f05_rpc_search_path.sql`** ⚠️ **PENDING prod Studio apply** + `f05_request_review_rpcs.sql` source updated; the approval-flow `UPDATE` status-guard Medium fixed across `leaves.js`/`expenses.js` (prior-status guards on the forward approve/reject paths + `.maybeSingle()` friendly errors; admin overrides left unguarded) — and while doing so, fixed a **latent bug** where the travel-claim override reused `approveTravelClaim` and silently coerced a `rejected`/`pending` override to `manager_approved` (now routed through a new `overrideTravelClaimStatus`); the redundant-empSelect-pre-filter Low aligned to "pass the full list" in `holidays-team.js`/`holidays-approvals.js`. **Still backlogged (genuinely needs live-DB access or is a design decision):** the corrected-severity RLS leave-approval finding (downgraded High→Medium on re-reading `holidays-approvals.js` — this app has no distinct HR role, so the two-tier flow is a process/audit-trail control, not a role boundary; the client status-guards now harden the JS path but a server-side trigger/RPC to stop a raw `curl` bypass needs Studio verification of current base-RLS grants first); the `travel_ref` count-then-insert race (needs a DB sequence/unique constraint); two conventions Lows that are design decisions to confirm rather than bugs (`employees.js` manager `<select>`, `team.js` account search box); and unwrapped UUID/serial primary-key attributes (deliberately not churned — no injection vector, wrapping purely for symmetry is noise). Items 2–4 from `NEXT_SESSION_PLAN.md` (re-run F-01 probe, spot-check Add-Member "App link", decide on the `Claude Design Import` GitHub App) all need the user's direct action — a cloud session cannot reach prod Supabase/Pages (confirmed 403 via the proxy gateway, same as prior rounds).)*
