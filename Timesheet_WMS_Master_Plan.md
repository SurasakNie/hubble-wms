...
| `20260716d_manager_create_projects.sql` | ⚠️ **PENDING Studio (R63 follow-up)** | **R63 follow-up** — walkthrough note "manager can't create a new project". The CREATE button already shows to managers; adds a PERMISSIVE `projects` INSERT policy for `get_my_role()='manager'` (admin unchanged; UPDATE/DELETE stay admin-only). Dump `pg_policies` for `projects` first. |
