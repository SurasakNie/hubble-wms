...
| `20260716c_mileage_total_distance.sql` | ✅ **2026-07-16 — applied (user-confirmed success; live trigger fn edited per the dump-first template)** | **R63** — removes the round-trip ×2 from the mileage-reimbursement trigger (entered distance is already the total). Trigger body isn't in the repo; the file is a dump-first template (get `pg_get_functiondef` for the travel_claims trigger fn, strip the ×2, `CREATE OR REPLACE`). Client preview already fixed in `js/api/expenses.js`. |
