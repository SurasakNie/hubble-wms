...
> **⏱ UPDATED 2026-07-13 (R62) — supersedes the block below.** **A4 (the pre-launch audit) is live and mid-run, user-driven:** **1A/1B/1C/1D done against real prod** (anon 61/61, client 41/41, member 5/5, manager scoping+bounce pass). Along the way, 1C/Phase-2 surfaced a real gap (Team page showed members the whole org) → built, shipped, and fully re-verified as **R61 Team-visibility scoping** (`20260713`/`20260713b`; RLS-enforced; all 4 re-audit legs green) — **✅ confirmed merged to `main` and live** (2026-07-13 — `app-init.js` on `main` reads `v=125`; corrects the "pending go-ahead" language that had been sitting in the docs). **This round (R62, docs+scripts only):** brought the remaining-phase audit materials current with R61's schema changes before anyone runs them live — Phase 1F gained the new `profiles_select`/helper-fn/trigger-fix SQL, Phase 2 gained an entirely new **2I · Team & Projects** subsection (had zero prior coverage), Phase 3 gained a `project_assignments` orphan check. **Update 2026-07-15: 1E + 1G done live — ✅ 18 PASS / 0 FAIL on `edge_probe.ps1`, incl. the credentialed non-admin-403/admin-malformed/1G-spoof checks.** Found + fixed **A4-F2** along the way: all 7 Edge Functions still whitelisted the dead `he-cells.github.io` CORS origin (R53 added the new one but never removed the old); fixed all 7 (no shared cors file — literal array in each, confirmed identical) + redeployed + re-verified. **1F also done — all green**, with 2 doc-naming errors found + fixed (real policy names `jtcr_select_own`/`jtcr_insert_own`/`audit_log_insert`, not what the doc guessed) and the trigger-check wording corrected (2 `task_assignments` rows is normal, not a fail). **▶ Next priority: 🧑/🌐 runs the remaining phases** — Phase 3 → Phase 2 (browser, incl. 2I) → Phase 4 (UI/UX + CSP console check) — **1A–1H all done**; user's chosen order for the rest, no dependency between them. **Phase 3 done** — core integrity clean (one item, #4, deliberately unscored since it's optional-by-design; flagged for 🧑, not a fail), Part Numbers P1–P4 clean, and a bug in 🤖's own query (assumed `project_assignments` had an `id` column) found + fixed. **▶ Next priority: 🧑/🌐 runs Phase 2** (browser, incl. 2I) **→ Phase 4** (UI/UX + CSP console check) — **requires a human with real network access or Supabase Studio**, still cannot run from this container (403 confirmed again this round). Full detail: [FULL_EXECUTION_PLAN.md](FULL_EXECUTION_PLAN.md) A4 · [PRE_LAUNCH_AUDIT_EXECUTION_PACKET.md](PRE_LAUNCH_AUDIT_EXECUTION_PACKET.md). **Also this session:** Track A's **A6** (closeout content — template wording + leave-policy decision) moved to Track B as **B10**, at the user's direction — it no longer gates A7/A8. **And: B1 (Timesheet → Central Hub) flagged 🔴 open** — the user's real billing process (hours × rate + expenses → invoice, no overtime) doesn't match B1's attendance/payroll scoping, and the core billing math turns out to already work today via Reports, independent of B1. **Deferred to the CEO — A7's agenda now formally includes a full Track B roadmap review**, not just functional/UX feedback on the shipped app. See B1's section in `FULL_EXECUTION_PLAN.md` for the full writeup. *(Block below kept for history; its framing is obsolete.)*
...
| `20260716_manager_scope_time_and_requests.sql` | ✅ **2026-07-16 — applied (user-confirmed success)** | **R63 walkthrough fix** — managers saw ZERO of their team's time entries / leave / flex / expense-travel rows (approvals + team calendar empty). Adds SECURITY DEFINER `is_my_report_emp()` + `my_employee_id()` and additive PERMISSIVE SELECT + UPDATE policies (own OR admin OR direct-report) on `time_entries` (user_id key) and `leave_requests`/`flex_holiday_swaps`/`cash_transactions`/`travel_claims`/`travel_requests` (employee_id key). **⚠️ Dump `pg_policies` for these 6 tables first** to reconcile names + confirm key columns (base schema not in repo). No wrapper. |
| `20260716b_employee_gender.sql` | ✅ **2026-07-16 — applied (user-confirmed success)** | **R63** — `employees.gender` (nullable, CHECK female/male) to scope maternity-leave eligibility. **Deploy order is load-bearing:** the v=126 frontend's `.select(..., gender)` on employees/holidays pages ERRORS if the column is absent, so this must be applied in Studio *before* the frontend push. |
| `20260716c_mileage_total_distance.sql` | ✅ **2026-07-16 — applied (user-confirmed success; live trigger fn edited per the dump-first template)** | **R63** — removes the round-trip ×2 from the mileage-reimbursement trigger. |
| `20260716d_manager_create_projects.sql` | ✅ **2026-07-16 — applied (user-confirmed success)** | **R63 follow-up** — walkthrough note "manager can't create a new project". The CREATE button already shows to managers; adds a PERMISSIVE `projects` INSERT policy for `get_my_role()='manager'` (admin unchanged; UPDATE/DELETE stay admin-only). Dump `pg_policies` for `projects` first. |
| `20260717_manager_group_member_scope.sql` | ✅ **2026-07-16 — applied (user-confirmed success)** | **R63 follow-up — supersedes 20260716's manager scope.** Re-scopes manager data visibility from direct-reports to **same-group members** per the role-tiered model: owner/admin = all; manager = own + same-group `role='member'` (see + approve), can see same-group admin/manager *names* but not their data; member = own only, no approve. Adds `shares_group_member()`/`shares_group_member_emp()`, rewrites the 6 `*_scope` policies, and adds an `employees` SELECT policy so the time-page picker lists same-group members. Dump `%_scope` + `employees` policies first. |
