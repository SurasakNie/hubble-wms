...
> **Entry point for this project. Start here.** *(Last updated: 2026-07-03)*
>
...
> **⏱ UPDATED 2026-07-03 (R54) — supersedes the blocks below.** **Working tree JS v=116 / CSS v=39→40** on branch `claude/next-session-plan-h9psa4` (PR #26, draft); **prod still serves the last merged main until #26 lands.** **Repo transferred `HE-cells` → `SurasakNie` (R53)** — prod is now `https://surasaknie.github.io/hubble-wms/`; full record in [REPO_TRANSFER_CHECKLIST.md](REPO_TRANSFER_CHECKLIST.md). **✅ F-01 (P0) CLOSED (R49)** — client RLS probe 22/22 PASS via `auth_is_client()` + `client_block_*` RESTRICTIVE policies (`20260708`, applied). Since R49: R50 CSP `<meta>` + inline-script externalization, R51 dropped SECURITY DEFINER view `client_project_totals`, R52 team.js App-link subpath fix, R53 repo transfer (+ fixed every Edge Fn's CORS allow-list, still hardcoded to the old domain — total login outage until fixed), **R54 full post-transfer security/code audit** ([AUDIT_2026-07-03_FULL_PROJECT.md](AUDIT_2026-07-03_FULL_PROJECT.md), local-only) — all Critical/High + most Medium/Low fixed (RLS: 12 more internal tables blocked incl. salary data; XSS: `projects.color` attribute-breakout; correctness: `copyLastWeek` ICT date shift; conventions: stale submodule pins, `.ts-remove-btn` opacity, `.emp-select-wrap` padding). **Edge Fns: 7.** **▶ Next priority (needs YOUR action — a cloud session can't reach prod):** apply the two pending migrations in Studio (`20260710_client_block_expanded.sql`, `20260711_f05_rpc_search_path.sql`) → re-run the F-01 probe → the live click-through test pass → **roster swap (LAST, RSK-0, 3 confirms).** **The single ordered checklist is in [NEXT_SESSION_PLAN.md](NEXT_SESSION_PLAN.md) § 6.** Details: [PENDING_TASKS.md](PENDING_TASKS.md) Round 54. *(Blocks below kept for history; their framing is obsolete.)*
