...
| `20260716_manager_scope_time_and_requests.sql` | ⚠️ **PENDING Studio (R63)** | **R63 walkthrough fix** — managers saw ZERO of their team's time entries / leave / flex / expense-travel rows (approvals + team calendar empty). Adds SECURITY DEFINER `is_my_report_emp()` + `my_employee_id()` and additive PERMISSIVE SELECT + UPDATE policies (own OR admin OR direct-report) on `time_entries` (user_id key) and `leave_requests`/`flex_holiday_swaps`/`cash_transactions`/`travel_claims`/`travel_requests` (employee_id key). **⚠️ Dump `pg_policies` for these 6 tables first** to reconcile names + confirm key columns (base schema not in repo). No wrapper. |
| `20260716b_employee_gender.sql` | ⚠️ **PENDING Studio — APPLY BEFORE deploying v=126** | **R63** — `employees.gender` (nullable, CHECK female/male) to scope maternity-leave eligibility. **Deploy order is load-bearing:** the v=126 frontend's `.select(..., gender)` on employees/holidays pages ERRORS if the column is absent, so this must be applied in Studio *before* the frontend push. |
| `20260716c_mileage_total_distance.sql` | ⚠️ **PENDING Studio — needs live DDL dump** | **R63** — removes the round-trip ×2 from the mileage-reimbursement trigger (entered distance is already the total). Trigger body isn't in the repo; the file is a dump-first template (get `pg_get_functiondef` for the travel_claims trigger fn, strip the ×2, `CREATE OR REPLACE`). Client preview already fixed in `js/api/expenses.js`. |
