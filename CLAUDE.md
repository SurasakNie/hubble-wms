...
- Current baseline after **Round 47** (SPEC-M1/M2/M3 close-out, 2026-06-30): **working JS `?v=111` / CSS `?v=39`**, tokens.css `?v=22`. **Next session: bump from v=111.** R47: **SPEC-M1/M2/M3 closed** — two-tier leave approval (`manager_approved` status, `approvalTiers` param in `approveLeaveRequest()`, AWAITING HR APPROVAL section in `holidays-approvals.js`); `STATUS_BADGE` in `holidays-state.js` extended with `manager_approved`; SPEC-M2 verified already satisfied by R44 RPC; SPEC-M3 — `requests.js` LEAVE REQUESTS badge now pending-only, PROFILE CHANGES converted to NAME CHANGES | JOB TITLE CHANGES sub-tabs. New migration `20260630_leave_manager_approved.sql` ⚠️ **PENDING prod Studio apply**. **STILL OPEN: F-01 (P0) — authenticated production client RLS probe is the go-live gate.** Prior R46: `?v=110`/CSS`?v=39`, CONV-M1 calendar weekNav. Prior R45: `?v=109`/CSS`?v=38`, audit log + client logins UX. Prior R44: `?v=107`, F-08/F-09/F-05 done. Prior R43: `?v=105`, module splits + UX batch. **Edge Fns: 7 deployed.** **Migrations APPLIED in prod Studio:** through `20260707` + `20260629_audit_log.sql`. **⚠️ PENDING prod Studio:** `20260630_leave_manager_approved.sql`. **Next session pending tasks:** apply `20260630_leave_manager_approved.sql` in Studio → F-01 (P0) prod client RLS probe (user action) → roster swap (RSK-0, 3 confirms). Prior live: R40 (`b647cdd` v=99); R39 (`6690861` v=98); R38 (`8f732e0` v=96); R35 (`3ff0449` v=94). Commits this session: `7884171` (R47 SPEC-M1/M2/M3) + docs close-out.
