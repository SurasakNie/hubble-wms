...
- Current baseline after **Round 45** (client logins UX + comprehensive audit log, 2026-06-29): **working JS `?v=109` / CSS `?v=38`**, tokens.css `?v=22`. **Next session: bump from v=109.** R45: **Client logins modal** — copy-credentials button after provision; Reset pw + Delete row actions added; modal widened to `modal-lg`. **Comprehensive audit log** — new `audit_log` table (`20260629_audit_log.sql`, **must be applied in prod Studio before going live**); new `js/api/auditLog.js` fire-and-forget helper; new `js/pages/adminLogs.js` admin page (filters: entity type, actor, date range; paginated table with action badges); `app.html` V→109 + `#admin-logs` route/nav/routeAllowed/wmsRoutes; `employees.js` History tab (reads `employee_audit_log`) + logAction for provision/reset-pw/clear-2FA/deactivate/reactivate; `clients.js` logAction for create/update/archive/restore/delete client + provision-login/reset-login-pw/delete-login; `holidays-approvals.js` logAction for approve/reject leave + flex swap; `expenses-approvals.js` logAction for approve/reject expense/claim/trip + settlement; `requests.js` logAction for approve/reject deletion/name-change/job-title-change. **STILL OPEN: F-01 (P0) — authenticated production client RLS probe is the go-live gate; `20260629_audit_log.sql` must be applied in prod Studio before audit log functions.** Prior R44: `?v=107`, F-08/F-09 done. Prior R43: `?v=102`, Help page. **Edge Fns: 7 deployed.** **Migrations APPLIED in prod Studio:** through `20260707` (audit log migration `20260629_audit_log.sql` PENDING — not yet applied). Prior live: R40 (`b647cdd` v=99); R39 (`6690861` v=98); R38 (`8f732e0` v=96); R35 (3ff0449, v=94); R29–R31 login overhaul (17edc56); R32–R34 (e284f50). Commit: `dc47bd7`.
